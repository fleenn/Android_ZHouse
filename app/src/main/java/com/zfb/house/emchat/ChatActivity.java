...
import com.zfb.house.component.AddFriendsView;
...
public class ChatActivity extends LemonActivity implements OnClickListener, EMEventListener {
    private ChatUserBean fromUserBean;//发送者
    private ChatUserBean toUserBean;//接收者
    /**
     *
     * @param activity
     * @param userType 0用户  1经纪人 2系统
     * @param userId
     * @param userNick
     * @param imgUrl
     */
    public static void launch(Context activity, int userType, String userId, String userNick, String imgUrl) {//"892bdc65059f43bb84c3eae006955f3e");//
        userType = userType - 1;
        Log.i("linwb","usertype  = " + userType);
        User user = new User();
        if (userNick != null) {
        ...
        chatUserBean.setUserName(userNick);
        chatUserBean.setImageUrl(imgUrl);
        Log.i("linwb","type === " + userType);
        chatUserBean.setUserType(userType);//类型
        intent.putExtra("chatUser", chatUserBean);
...
//    public static final String USER_TYPE = "userType";//true经纪人  false客户
//    public static final String T_TYPE = "tType";//接收者的类型
//    public static final String F_TYPE = "fType";//发送者的类型
...
//	private GroupListener groupListener;
    ...
    private AddFriendsView avAddFriends;
    ...
    protected void initView() {
        ...
        avAddFriends = (AddFriendsView) findViewById(R.id.add_frends);
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if (listView.getFirstVisiblePosition() == 0 && !isloading && haveMoreData) {
                            ...
                            Toast.makeText(ChatActivity.this, "没有更多的消息了", Toast.LENGTH_SHORT).show();
                        }
                        swipeRefreshLayout.setRefreshing(false);
                    }
                }, 1000);
        ...
        if (TextUtils.isEmpty(judueFriends())){
            avAddFriends.setVisibility(View.VISIBLE);
            avAddFriends.setHeaderAndUid(getIntent().getStringExtra("imgUrl")
                    ,getIntent().getStringExtra("userId"),toChatUsername,getIntent().getStringExtra("userType"));
        }
    }
    ...
    private void setUpView() {
        ...
        /*if(toUserBean==null){//如果是空的话代表这个时候是从历史消息界面里面传过去的这个时候重新设置值
    ...
    protected void onConversationInit() {
        if (TextUtils.isEmpty(toChatUsername)) {
            LogUtils.e(TAG, "toChatUsername == null , 对方用户名为空");
            return;
        }
        if (chatType == CHATTYPE_SINGLE) {
    ...
    protected void onListViewCreation() {
        if (TextUtils.isEmpty(toChatUsername)) {
            LogUtils.e(TAG, "toChatUsername == null");
            return;
        }
        adapter = new MessageAdapter(ChatActivity.this, toChatUsername, chatType);
        userType = getIntent().getStringExtra("userType");
        Log.i("linwb3","use type 2 = "  + userType);
        userId = getIntent().getStringExtra("userId");
        adapter.setUserFromInfo(userId,userType);
        // 显示消息
...
/*	protected void onGroupViewCreation(){
	...
	protected void onChatRoomViewCreation(){
            ...
            // TODO Auto-generated method stub
             runOnUiThread(new Runnable(){
                   ...
                   public void run(){
                        ...
                        ELogUtils.d(TAG, "join room success : " + room.getName());
        ...
        public void onError(final int error, String errorMsg) {
                // TODO Auto-generated method stub
                ELogUtils.d(TAG, "join room failure : " + error);
               runOnUiThread(new Runnable(){
                   @Override
                   public void run(){
                       pd.dismiss();
        ...
        if (resultCode == RESULT_OK) { // 清空消息
            if (requestCode == REQUEST_CODE_EMPTY_HISTORY) {
                ...
                try {
                    ...
                    if (bitmap == null) {
                        LogUtils.d("chatactivity", "problem load video thumbnail bitmap,use default icon");
...
//			startActivityForResult(new Intent(this, BaiduMapActivity.class), REQUEST_CODE_MAP);
    ...
    public void selectPicFromCamera() {
        ...
        cameraFile = new File(PathUtil.getInstance().getImagePath(), UserBean.getInstance(this).name/*DemoApplication.getInstance().getUserName()*/
    ...
    public void sendText(String content) {
        ...
        LogUtils.e("ChatActivity", "toChatUsername==" + toChatUsername);
        if (content.length() > 0) {
...
//            message.setFrom(UserBean.getInstance(mContext).name);
            // 把messgage加到conversation中 添加属性
    ...
    private void addAttribute2Msg(EMMessage message) {
        Gson gson = new Gson();
        if (toUserBean != null) {
            message.setAttribute(TO_USER, gson.toJson(toUserBean));
        }
        UserBean localUserBean = UserBean.getInstance(this);
        fromUserBean.setImageUrl(localUserBean.photo);
        Log.i("linwb","for = " + toUserBean.getUserType() + " form = " + fromUserBean.getUserType());
        message.setAttribute(FROM_USER, gson.toJson(fromUserBean));
