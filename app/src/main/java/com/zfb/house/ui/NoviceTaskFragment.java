 ...
 */
@Layout(id = R.layout.fragment_task_novice)
public class NoviceTaskFragment extends LemonFragment {

    private static final String TAG = "NoviceTaskFragment";

    //每日任务列表
    @FieldView(id = R.id.lv_novice_task)
    private ListView lvNoviceTask;
    ...
    public void onEventMainThread(DayTaskResult result) {
        ...
        if (result.getResultCode().equals(StatusCode.SUCCESS.getCode())) {
            ...
            lvNoviceTask.setAdapter(new DayTaskAdapter(getActivity(), data));
        } else {
