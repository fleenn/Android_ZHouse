 ...
 */
public class CollectRentAdapter extends RecyclerView.Adapter {

    private List<RentItem> models;//数据
    ...
    public interface OnRecycleViewListener {
        void onItemClick(View view, int position, RentItem item);//item的监听

        void onDeleteClick(View view, int position, RentItem item);//删除按钮的监听
    }
    ...
    public void setData(List<RentItem> models) {
        this.models = models;
    }

    public void addData(List<RentItem> models) {
        this.models.addAll(models);
    }

    public List<RentItem> getData() {
        return models;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, final int position) {
        ...
        if (!ParamUtils.isNull(models)) {
            final RentItem model = this.models.get(position);
            //房子照片
            ...
            //设置出租的字段
            holder.txtType.setText(ToolUtil.setRentType(model));
            //面积
            ...
            if (!ParamUtils.isEmpty(model.getRental())) {
                holder.txtPrice.setText(model.getRental() + "元/月");
            }
