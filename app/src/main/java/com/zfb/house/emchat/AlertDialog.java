...
public class AlertDialog extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(path != null){
		    ...
		    if(ImageLoader.ImageCache.getInstance().get(path) != null){
		        imageView.setImageBitmap(ImageLoader.ImageCache.getInstance().get(path));
		    }else{
		        Bitmap bm = ImageUtils.decodeScaleImage(path, 150, 150);
		        imageView.setImageBitmap(bm);
		        ImageLoader.ImageCache.getInstance().put(path, bm);
		    }*/
