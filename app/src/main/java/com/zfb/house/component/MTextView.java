...
//	private float lineSpacingMult = 0.5f;
	...
	private OnLinkSpanClickListener mOnLinkSpanClickListener;

	public MTextView(Context context) {super(context);init(context);}
	public MTextView(Context context, AttributeSet attrs) {super(context, attrs);init(context);}
	public MTextView(Context context, AttributeSet attrs, int defStyleAttr) {super(context, attrs, defStyleAttr);init(context);}
	@TargetApi(Build.VERSION_CODES.LOLLIPOP) public MTextView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {super(context, attrs, defStyleAttr, defStyleRes);init(context);}
		...
		for (int i = 0; i < contentList.size(); i++){
			...
			for (int j = 0; j < line.line.size(); j++){
				...
				if (ob instanceof ImageSpan){
					...
					paint.setColor(textColor);
					float topX = realDrawedWidth;
					float topY = height + line.height;
					if(ob instanceof StringWithColor){
						StringWithColor stringWithColor = (StringWithColor) ob;
						paint.setColor(stringWithColor.color);
						stringWithColor.left = topX;
						stringWithColor.top = topY -  line.height - lineSpacing;
						stringWithColor.right = topX + width;
						stringWithColor.bottom = stringWithColor.top + line.height;
					}
					canvas.drawText(textStr, topX, topY, paint);
					realDrawedWidth += width;
				}
			}
	...
	public boolean onTouchEvent(MotionEvent event) {
		if(mOnLinkSpanClickListener != null){
			switch (event.getAction()){
				case MotionEvent.ACTION_DOWN:
					if(contentList != null && !contentList.isEmpty()){
						float downX = event.getX();
						float downY = event.getY();
						for(LINE line : contentList){
							if(line != null){
								ArrayList<Object> objects = line.line;
								if(objects != null){
									for(Object ob : objects){
										if(ob != null && ob instanceof StringWithColor){
											StringWithColor stringWithColor  = (StringWithColor)ob;
											if(downX >= stringWithColor.left && downX <= stringWithColor.right
													&& downY >= stringWithColor.top && downY <= stringWithColor.bottom ){
												Log.v(TAG , "MtextView onTouch down click char: "+ob.toString()+"," +
														" content: "+stringWithColor.wholeContent);
												mOnLinkSpanClickListener.onClick(MTextView.this , stringWithColor.wholeContent);
												return true;
											}
										}
									}
	...
	public void setOnLinkSpanClickListener(OnLinkSpanClickListener onLinkSpanClickListener){
		mOnLinkSpanClickListener = onLinkSpanClickListener;
	}
	...
	public void setMText(CharSequence textStr) {
		...
		if (textStr instanceof SpannableString) {
			...
			for (int i = 0; i < spanneds.length; i++) {
				...
				spanBean.start = s;
				spanBean.end = e;
				spanBean.str = ss.subSequence(s,e).toString();
				isList.add(spanBean);
			}
			...
			if (j < isList.size()) {
				...
				if (i < spanBean.start) {
					...
					if(spanBean instanceof IS){
						...
						obList.add(new StringWithColor(
								new String(Character.toChars(cp)),
								((ForegroundColorSpanBean)spanBean).fcspan.getForegroundColor(),
								((ForegroundColorSpanBean)spanBean).str));
						if(i >= spanBean.end){
							j++;
						}
					}else{
	...
	class StringWithColor{
		public float left , top , right ,bottom;//存储指定字符或文本的点击事件的坐标范围
		private int id;
		int color = 0;
		String str;//需要绘制的字符
		String wholeContent;//所对应的文本内容
		StringWithColor( String str , int color , String wholeContent){
			this.str = str;
			this.color = color;
			this.wholeContent = wholeContent;
			this.id = (wholeContent + str).hashCode();
		}
