...
/**
 * （经纪人）接受的委托：呼叫委托、房源委托、聊天委托
 * Created by Administrator on 2016/5/24.
 */
@Layout(id = R.layout.activity_entrust)
public class BrokerEntrustActivity extends LemonActivity {
    ...
    //呼叫委托
    private BrokerCallEntrustFragment brokerCallEntrustFragment;
    //房源委托
    private BrokerHouseEntrustFragment brokerHouseEntrustFragment;
    //聊天委托
    private AcceptedHouseFragment acceptedHouseFragment;

    public void setIsEdit(boolean isEidt) {
        this.isEdit = isEidt;
    }
    ...
    public void toEdit() {//编辑
        ...
        if (pagerEntrustSchema.getCurrentItem() == 1) {
            brokerHouseEntrustFragment.editRefreshView(isEdit);
        } else if (pagerEntrustSchema.getCurrentItem() == 0) {
            brokerCallEntrustFragment.editRefreshView(isEdit);
        }
    }

    @Override
    protected void initView() {
        setCenterText(R.string.img_mine_accept_entrust);
        setRtText(R.string.edit);
        //默认房源委托
        txtHouse.performClick();
        mFragments = new ArrayList<>();
        brokerCallEntrustFragment = new BrokerCallEntrustFragment();
        brokerHouseEntrustFragment = new BrokerHouseEntrustFragment();
        acceptedHouseFragment = new AcceptedHouseFragment();
        mFragments.add(brokerCallEntrustFragment);
        mFragments.add(brokerHouseEntrustFragment);
        mFragments.add(acceptedHouseFragment);
    ...
    private void initCursorPos() {
        if (ScreenUtil.screenWidth == 0) {
            ScreenUtil.init(mContext); //初始化动画
        }
