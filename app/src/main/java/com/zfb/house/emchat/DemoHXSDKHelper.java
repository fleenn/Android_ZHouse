...
import com.zfb.house.ui.MainActivity;
        ...
        eventListener = new EMEventListener() {
            ...
            public void onEvent(EMNotifierEvent event) {
                ...
                if (event.getData() instanceof EMMessage) {
                    ...
                    EMLog.d(TAG, "receive the event : " + event.getEvent() + ",id : " + message.getMsgId() + "name = " +  message.getFrom());
                }
                    ...
                    case EventNewMessage:
                        ...
                        if (activityList.size() <= 0) {
                            ...
                            if(activity instanceof MainActivity){
                                ...
                                ((MainActivity)activity).refreshChatList(message);
                            }
                            }
                        if(message!=null){
                            if(message.getFrom().equals("admin")){
                                if("2".equals(message.getStringAttribute("code",""))){//2的话为抢单消息
                                    for(Activity activity:activityList){
                                       /* if(activity instanceof TabActivity){
                                            ((TabActivity)activity).setAkeyOrderRed(message);
                                            break;
                                        }*/
                                    }
                                }
    ...
    protected HXNotifier.HXNotificationInfoProvider getNotificationListener() {
        ...
        return new HXNotifier.HXNotificationInfoProvider() {
            ...
            public String getDisplayedText(EMMessage message) {
                ...
                LogUtils.e("HXNotification", "getDisplayedText()" + "自定义通知栏提示内容:" + message.toString());
...
//                    return message.getFrom() + ": " + ticker;
            ...
            public Intent getLaunchIntent(EMMessage message) {
                //设置点击通知栏跳转事件
                LogUtils.e("HXNotification", "getLaunchIntent()" + "自定义通知栏提示内容:" + message.toString());
    ...
    protected void onConnectionConflict() {
        LemonContext.getBean(LemonMessage.class).sendMessage(R.string.connect_conflict);
        LogUtils.e("DemoHXSDKHelper", "同一账号登录");
        EMChatManager.getInstance().logout();//此方法为同步推出
        SettingUtils.set(appContext, "token", "");
        UserBean.clearUserBean(appContext);
        Intent intent = new Intent(appContext, LoginActivity.class);
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        intent.putExtra("conflict", true);
        Activity currentActivity = LemonContext.getBean(LemonActivityManager.class).getCurrentActivity();
        EventUtil.sendEvent(new RefreshEvent());
        EventUtil.sendEvent(new StopJpushEvent());
...
//        ToastUtil.showToast(appContext,appContext.getResources().getString(R.string.user_removed));
//        Intent intent = new Intent(appContext, LoginActivity.class);
//        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//        intent.putExtra(Constant.ACCOUNT_REMOVED, true);
//        appContext.startActivity(intent);
        LogUtils.e("DemoHXSDKHelper","账号被移除了");
//        UserManager.logout(appContext, "");
    }
